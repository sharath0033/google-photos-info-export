...
app.post('/export', async (req, res) => {
  const imagePaths = await recFindByExt(process.env.SEARCH_DIRECTORY, req.body);
  if(imagePaths && imagePaths.length !== req.body){
    req.body.forEach(item => {
      const imageNot = imagePaths.find(imageObj => imageObj.fileName === item.fileName);
      if(!imageNot){
        imagePaths.push({
          sno: (imagePaths.length + 1),
  ...
  res.status(200).send({
    headers: ['SNo', 'File Name', 'Camera', 'Created Date', 'Path', 'Full Path'],
    items: imagePaths || []
  });
});

async function recFindByExt(base, filesList, files, result) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(newbase).isDirectory()){
        result = await recFindByExt(newbase, filesList, fs.readdirSync(newbase), result);
    } else {
          ...
          for (const gFile of filterList) {
            ...
            if (gFile.fileName === file && gFile.cameraModel === fileInfo.Model && gFileTimeStamp === fileTimeStamp) {
              result.push({
                sno: (result.length + 1),
                fileName: file,
                camera: gFile.cameraModel,
                createdDate: gFile.creationTime,
